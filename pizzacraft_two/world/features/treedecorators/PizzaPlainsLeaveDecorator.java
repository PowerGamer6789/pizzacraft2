...
package net.mod.pizzacraft_two.world.features.treedecorators;
...
public class PizzaPlainsLeaveDecorator extends LeaveVineDecorator {
	public static MapCodec<PizzaPlainsLeaveDecorator> CODEC = MapCodec.unit(PizzaPlainsLeaveDecorator::new);
	public static TreeDecoratorType<?> DECORATOR_TYPE = new TreeDecoratorType<>(CODEC);

	@SubscribeEvent
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(Registries.TREE_DECORATOR_TYPE, ResourceLocation.parse("pizzacraft2:pizza_plains_tree_leave_decorator"), () -> DECORATOR_TYPE);
	}

	public PizzaPlainsLeaveDecorator() {
		super(0.25f);
	}
	...
	private static void addVine(BlockPos pos, Direction direction, TreeDecorator.Context context) {
		context.setBlock(pos, Pizzacraft2ModBlocks.CHEESE_LEAVES.get().defaultBlockState());
		int i = 4;
		for (BlockPos blockpos = pos.below(); context.isAir(blockpos) && i > 0; --i) {
			context.setBlock(blockpos, oriented(Pizzacraft2ModBlocks.CHEESE_LEAVES.get().defaultBlockState(), direction));
